...
Most production systems already emit plenty of metrics: queue depth, consumer offset drift, processing rates, and more. The hard part is rarely collection—it is **decision latency**. A backlog of half a million items might be harmless under high throughput, or the first visible sign of a stall. Raw numbers force every on-call to re-derive context under pressure.
...
- **Sampler** pulls lightweight snapshots of authoritative operational state (for example backlog, offsets, and processing counters) on a fixed cadence.
...
To support many concurrent viewers without N× recomputation, a **Redis-based fan-out** broadcasts updates: the signal engine computes once per material change; subscribers receive the same payload. That pattern keeps CPU bounded as audience size grows and preserves a consistent operational narrative across the room during an incident.
...
- lag and offset deltas become **estimated time to catch up**;
